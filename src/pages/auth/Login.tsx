...
const Login = ({ usersService, storageService }: props) => {
  ...
  const [makeToast, _, dismissToast] = useContext(
    ToastContext,
  ) as ToastContextType;
  ...
  const onLogin: SubmitHandler<FormData> = async ({ email, password }) => {
    ...
    if (user) {
      ...
      setLoggedUser({
        ...
        spoonacularUser: {
          username: (user as User).spoonacularUser.username,
          hash: (user as User).spoonacularUser.hash,
        },
      });
    } else {
      makeToast!(
        'User does not exist or wrong combination of username or password',
        'error',
      );
    }
